...
const AuthManager = {
    ...
    activateKey: function(inputKey) {
        ...
        // ★★★ マスターキーの設定 ★★★
        if (cleanedKey === "TOKYO-MATH-MASTER") { 
            localStorage.setItem('full_access', 'true');
            // すべての大問セットも念のため個別にONにする
            localStorage.setItem('daimon1_all', 'true');
            localStorage.setItem('daimon2_all', 'true');
            localStorage.setItem('daimon3_all', 'true');
            localStorage.setItem('daimon4_all', 'true');
            localStorage.setItem('daimon5_all', 'true');
            return true;
        }
        ...
        const target = keyMap[cleanedKey];
        if (target) {
    ...
    // 【おまけ】デバッグ用：すべてのロックをリセットする
    resetAll: function() {
        localStorage.clear();
        alert("すべてのライセンスをリセットしました。");
        location.reload();
    }
};
/**
